/**
 * @file RobinBoundary_Selector.tpl.cc
 *
...
#include <combustion/RobinBoundary/RobinBoundaries.hh>
...
create_function(
    ...
    DTM::pout << "* found configuration: robin_boundary function = " << _type << std::endl;
    DTM::pout << "* found configuration: robin_boundary options = " << std::endl;
    ...
    if (_type.compare("ZeroFunction") == 0) {
        AssertThrow(
            options.size() == 0,
            dealii::ExcMessage(
                "robin_boundary options invalid, "
        ...
        DTM::pout
            << "robin_boundary selector: created zero function" << std::endl
            << std::endl;
    ...
    if (_type.compare("ConstantFunction") == 0) {
        ...
        DTM::pout
            << "robin_boundary selector: created ConstantFunction "
            << "as robin_boundary function, with " << std::endl
            << "\tg_1(x) = " << values.at(0) << " , " << std::endl
            << "\tg_2(x) = " << values.at(1) << " . " << std::endl
            << std::endl;
    ...
    AssertThrow(
        false,
        dealii::ExcMessage("robin_boundary function unknown, please check your input file data.")
    );
}

}} //namespaces

#include "RobinBoundary_Selector.inst.in"
